...
class MPBloc extends Bloc<MPEvent, MPState> {
  ...
  Stream<MPState> _mapEditMPToState(EditMP event) async* {
    if (_isEventAttributeIsNotNull(event)) {
      yield _loading();
      MaterialPurchase mp = _materialPurchase(event);
      ResponseResult result = await _mpServices.editMaterialPurchaseByCode(mp);
...
// }
  ...
  Stream<MPState> _mapDeleteMPToState(DeleteMP deleteMP) async* {
    Map<String, String> mpcode = {'mpcode': deleteMP.mpcode};
    ResponseResult result = await _mpServices.deleteMaterialPurchase(mpcode);
    if (result.status == true) {
      yield _success(result);
    } else {
      yield _error(result);
